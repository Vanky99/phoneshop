...
<div class="row justify-content-center">
    <div class="col-lg-8 col-md-12 p-4">
        ...
        <div class="my-4">
            ...
            ?>
                    <div id="cart-item-<?=$cart['id']?>" class="row m-0 p-2 cart-items">
                        <div class="col-md-8 col-sm-12">
                            <div class="d-flex align-items-center">
                                <div style="background-image: url('<?='public/images/phones/phone' . $cart['id']. '.png'?>')" class="phone-image"></div>
                                <div class="m-3">
                                    <h3><?=$cart['name']?></h3>
                                    <div>
                                        <input class="carts-price" type="hidden" value="<?=$cart['price'] - ($cart['price'] * $cart['discount'] / 100)?>" />
                                        <h4 class="text-danger"><?=number_format($cart['price'] - ($cart['price'] * $cart['discount'] / 100))?>đ</h4>
                        ...
                        <div class="col-md-4 col-sm-12">
                            <div class="d-flex flex-column justify-content-center h-100">
                                <div class="btn-group mb-3" role="group">
                                    <button type="button" class="btn btn-danger cart-sub" value="<?=$cart['id']?>">-</button>
                                    <button id="cart-quantity-<?=$cart['id']?>" type="button" class="btn btn-light disabled px-5"><?=$_SESSION['carts'][$index]['quantity']?></button>
                                    <button type="button" class="btn btn-danger cart-add" value="<?=$cart['id']?>">+</button>
                                </div>
        ...
        <h4 id="total-price" class="text-center">Tổng tiền: <?=number_format($sum)?>đ</h4>
        <!--End of carts-->
        <div class="d-flex justify-content-center">
            <button class="btn btn-danger my-3" data-bs-toggle="modal" data-bs-target="#bill"><h4>Đặt hàng</h4></button>
        </div>
    </div>
</div>
<!--End of container-->
