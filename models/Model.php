...
class Model {
    ...
    public function index(string $select = '*', string $join = '', string $where = '', int $page = 1, int $row_per_page = 10, $add = '') : mysqli_result {
        $offset = ($page-1) * $row_per_page;
        $join_string = empty($join) ? '' : "JOIN $join";
        $where_string = empty($where) ? '' : "WHERE $where";
        $command = "SELECT $select FROM $this->table $join_string $where_string $add LIMIT $row_per_page OFFSET $offset";
        //echo $command;
        return $this->db->query($command);
    }
    ...
    public function show(string $select = '*', string $join = '', string $where = '') : array {
        ...
        $where_string = "$this->table.$this->primary_key = '" . $this->{$this->primary_key} . "' $where_and_string";
